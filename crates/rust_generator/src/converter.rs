...
pub fn package_to_proto(package: &Package) -> Result<pulumi::Package> {
    ...
    for (id, global_type) in &package.types {
        let global_type_value =  global_type_to_proto(global_type).context("Failed to convert global type to proto")?;
...
pub fn proto_to_package(proto: &pulumi::Package) -> Result<Package> {
    ...
    for (_, proto_resource) in &proto.resources {
        let resource = proto_to_resource(proto_resource)
            .context("Failed to convert proto to resource")?;
        resources.insert(resource.element_id.clone(), resource);
    }

    // Convert functions
    let mut functions = BTreeMap::new();
    for (_, proto_function) in &proto.functions {
        let function = proto_to_function(proto_function)
            .context("Failed to convert proto to function")?;
        functions.insert(function.element_id.clone(), function);
    }

    // Convert types
    let mut types = BTreeMap::new();
    for (_, proto_type) in &proto.types {
        let (id, global_type) = proto_to_global_type(proto_type)
            .context("Failed to convert proto to global type")?;
...
fn proto_to_resource(proto: &pulumi::Resource) -> Result<Resource> {
    let element_id = proto_to_element_id(proto.element_id.as_ref()
...
fn proto_to_input_property(proto: &pulumi::InputProperty) -> Result<InputProperty> {
    Ok(InputProperty {
        name: proto.name.clone(),
        r#type: proto_to_type(proto.r#type.as_ref()
...
fn proto_to_type(proto: &pulumi::Type) -> Result<Type> {
    let type_value = proto.type_value.as_ref()
    ...
    match type_value {
        ...
        pulumi::r#type::TypeValue::ArrayType(t) => {
            Ok(Type::Array(Box::new(proto_to_type(t)?)))
        },
        pulumi::r#type::TypeValue::ObjectType(t) => {
            Ok(Type::Object(Box::new(proto_to_type(t)?)))
        },
        pulumi::r#type::TypeValue::RefType(r) => {
            Ok(Type::Ref(proto_to_ref(r)?))
        },
        pulumi::r#type::TypeValue::OptionType(t) => {
            Ok(Type::Option(Box::new(proto_to_type(t)?)))
        },
        ...
        pulumi::r#type::TypeValue::ConstString(s) => {
            Ok(Type::ConstString(s.clone()))
        }
    }
}
...
fn proto_to_ref(proto: &pulumi::RefType) -> Result<Ref> {
    let ref_value = proto.ref_value.as_ref()
        .context("RefType proto missing ref_value")?;

    match ref_value {
        pulumi::ref_type::RefValue::TypeRef(id) => {
            Ok(Ref::Type(proto_to_element_id(id)))
        },
...
fn proto_to_global_type(proto: &pulumi::GlobalType) -> Result<(ElementId, GlobalType)> {
    ...
    let global_type_value = proto.global_type_value.as_ref()
...
fn proto_to_global_type_property(proto: &pulumi::GlobalTypeProperty) -> Result<GlobalTypeProperty> {
    Ok(GlobalTypeProperty {
        name: proto.name.clone(),
        r#type: proto_to_type(proto.r#type.as_ref().context("GlobalTypeProperty proto missing type")?)?,
        description: proto.description.clone(),
    })
}
