...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for AppProductionBranch {
    ...
    > {
        async move {
            ...
            map.insert("last_deploy_time".to_string(), self.r#last_deploy_time.to_pulumi_value().await);
            map.insert("status".to_string(), self.r#status.to_pulumi_value().await);
            map.insert("thumbnail_url".to_string(), self.r#thumbnail_url.to_pulumi_value().await);
