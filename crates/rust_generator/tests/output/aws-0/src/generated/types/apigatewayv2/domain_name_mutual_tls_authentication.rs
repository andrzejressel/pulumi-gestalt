...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for DomainNameMutualTlsAuthentication {
    ...
    > {
        async move {
            ...
            map.insert("truststore_uri".to_string(), self.r#truststore_uri.to_pulumi_value().await);
            map.insert("truststore_version".to_string(), self.r#truststore_version.to_pulumi_value().await);
