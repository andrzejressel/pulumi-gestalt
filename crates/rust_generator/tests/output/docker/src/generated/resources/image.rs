...
pub mod image {
    ...
    fn __create(
        ...
        let build_on_preview_binding = args.build_on_preview.get_output(ctx);
        let image_name_binding = args.image_name.get_output(ctx);
        let registry_binding = args.registry.get_output(ctx);
        let skip_push_binding = args.skip_push.get_output(ctx);
