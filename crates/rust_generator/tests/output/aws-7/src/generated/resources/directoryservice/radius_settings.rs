...
pub mod radius_settings {
    ...
    fn __create(
        ...
        let display_label_binding = args.display_label.get_output(ctx);
        let radius_port_binding = args.radius_port.get_output(ctx);
        let radius_retries_binding = args.radius_retries.get_output(ctx);
        let radius_servers_binding = args.radius_servers.get_output(ctx);
        let radius_timeout_binding = args.radius_timeout.get_output(ctx);
        let shared_secret_binding = args.shared_secret.get_output(ctx);
        let use_same_username_binding = args.use_same_username.get_output(ctx);
