        ...
        async move {
            ...
            let field_futures: Vec<ToPulumiObjectFieldFuture<'_>> = vec![
                to_pulumi_object_field(
                    "captureInterval",
                    &self.r#capture_interval,
                ),
                to_pulumi_object_field(
                    "captureIntervalUnits",
                    &self.r#capture_interval_units,
                ),
            ];
    ...
    fn from_pulumi_value(
        ...
        match value.content {
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    r#capture_interval: {
                        let field_value = match fields_map.get("captureInterval") {
                            Some(value) => value,
                            None => bail!("Missing field 'captureInterval' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#capture_interval_units: {
                        let field_value = match fields_map.get("captureIntervalUnits") {
                            Some(value) => value,
                            None => bail!("Missing field 'captureIntervalUnits' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                })
