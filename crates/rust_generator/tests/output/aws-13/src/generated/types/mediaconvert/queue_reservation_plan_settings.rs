...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for QueueReservationPlanSettings {
    ...
    > {
        async move {
            ...
            map.insert("commitment".to_string(), self.r#commitment.to_pulumi_value().await);
            map.insert("renewal_type".to_string(), self.r#renewal_type.to_pulumi_value().await);
            map.insert("reserved_slots".to_string(), self.r#reserved_slots.to_pulumi_value().await);
