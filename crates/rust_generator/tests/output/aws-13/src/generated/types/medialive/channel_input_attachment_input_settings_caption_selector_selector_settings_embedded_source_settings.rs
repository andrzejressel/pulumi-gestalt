...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for ChannelInputAttachmentInputSettingsCaptionSelectorSelectorSettingsEmbeddedSourceSettings {
    ...
    > {
        async move {
            ...
            map.insert("convert_608_to_708".to_string(), self.r#convert_608_to_708.to_pulumi_value().await);
            map.insert("scte_20_detection".to_string(), self.r#scte_20_detection.to_pulumi_value().await);
            map.insert("source_608_channel_number".to_string(), self.r#source_608_channel_number.to_pulumi_value().await);
