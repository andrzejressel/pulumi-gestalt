        ...
        async move {
            ...
            let field_futures: Vec<ToPulumiObjectFieldFuture<'_>> = vec![
                to_pulumi_object_field(
                    ...
                    &self.r#encryption_at_rest_kms_key_arn,
                ),
                to_pulumi_object_field(
                    "encryptionInTransit",
                    &self.r#encryption_in_transit,
                ),
            ];
            ...
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    r#encryption_at_rest_kms_key_arn: {
                        let field_value = match fields_map.get("encryptionAtRestKmsKeyArn") {
                            Some(value) => value,
                            None => bail!("Missing field 'encryptionAtRestKmsKeyArn' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#encryption_in_transit: {
                        let field_value = match fields_map.get("encryptionInTransit") {
                            Some(value) => value,
                            None => bail!("Missing field 'encryptionInTransit' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                })
