        ...
        match value.content {
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    ...
                    r#excluded_packages: {
                        let field_value = match fields_map.get("excludedPackages") {
                            Some(value) => value,
                            None => bail!("Missing field 'excludedPackages' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#included_packages: {
                        let field_value = match fields_map.get("includedPackages") {
                            Some(value) => value,
                            None => bail!("Missing field 'includedPackages' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#reboot: {
