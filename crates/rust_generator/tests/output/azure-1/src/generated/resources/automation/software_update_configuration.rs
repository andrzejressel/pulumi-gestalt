...
pub mod software_update_configuration {
    ...
    fn __create(
        ...
        let linux_binding = args.linux.get_output(ctx);
        let name_binding = args.name.get_output(ctx);
        let non_azure_computer_names_binding = args
            .non_azure_computer_names
            .get_output(ctx);
        let post_task_binding = args.post_task.get_output(ctx);
        let pre_task_binding = args.pre_task.get_output(ctx);
        let schedule_binding = args.schedule.get_output(ctx);
        let target_binding = args.target.get_output(ctx);
        let virtual_machine_ids_binding = args.virtual_machine_ids.get_output(ctx);
