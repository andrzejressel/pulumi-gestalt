...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for ConnectorProfileConnectorProfileConfigConnectorProfilePropertiesSapoDataOauthProperties {
    ...
    > {
        async move {
            ...
            map.insert("auth_code_url".to_string(), self.r#auth_code_url.to_pulumi_value().await);
            map.insert("oauth_scopes".to_string(), self.r#oauth_scopes.to_pulumi_value().await);
            map.insert("token_url".to_string(), self.r#token_url.to_pulumi_value().await);
