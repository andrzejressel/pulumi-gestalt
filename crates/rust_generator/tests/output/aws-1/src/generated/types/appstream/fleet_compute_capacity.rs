        ...
        match value.content {
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    ...
                    r#desired_instances: {
                        let field_value = match fields_map.get("desiredInstances") {
                            Some(value) => value,
                            None => bail!("Missing field 'desiredInstances' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#desired_sessions: {
                        let field_value = match fields_map.get("desiredSessions") {
                            Some(value) => value,
                            None => bail!("Missing field 'desiredSessions' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#in_use: {
                        let field_value = match fields_map.get("inUse") {
                            Some(value) => value,
                            None => bail!("Missing field 'inUse' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#running: {
