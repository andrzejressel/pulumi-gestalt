...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for FlowDestinationFlowConfigDestinationConnectorPropertiesUpsolverS3OutputFormatConfigPrefixConfig {
    ...
    > {
        async move {
            ...
            map.insert("prefix_format".to_string(), self.r#prefix_format.to_pulumi_value().await);
            map.insert("prefix_hierarchies".to_string(), self.r#prefix_hierarchies.to_pulumi_value().await);
            map.insert("prefix_type".to_string(), self.r#prefix_type.to_pulumi_value().await);
