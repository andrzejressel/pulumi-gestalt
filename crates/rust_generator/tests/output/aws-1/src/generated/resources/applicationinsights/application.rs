...
pub mod application {
    ...
    fn __create(
        ...
        let auto_config_enabled_binding = args.auto_config_enabled.get_output(ctx);
        let auto_create_binding = args.auto_create.get_output(ctx);
        let cwe_monitor_enabled_binding = args.cwe_monitor_enabled.get_output(ctx);
        let grouping_type_binding = args.grouping_type.get_output(ctx);
        let ops_center_enabled_binding = args.ops_center_enabled.get_output(ctx);
        let ops_item_sns_topic_arn_binding = args.ops_item_sns_topic_arn.get_output(ctx);
