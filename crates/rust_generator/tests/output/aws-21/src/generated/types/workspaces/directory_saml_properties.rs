...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for DirectorySamlProperties {
    ...
    > {
        async move {
            ...
            map.insert("relay_state_parameter_name".to_string(), self.r#relay_state_parameter_name.to_pulumi_value().await);
            map.insert("status".to_string(), self.r#status.to_pulumi_value().await);
            map.insert("user_access_url".to_string(), self.r#user_access_url.to_pulumi_value().await);
