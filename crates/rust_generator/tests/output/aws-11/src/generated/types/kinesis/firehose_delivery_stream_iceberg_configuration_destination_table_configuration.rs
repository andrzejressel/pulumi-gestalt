...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for FirehoseDeliveryStreamIcebergConfigurationDestinationTableConfiguration {
    ...
    > {
        async move {
            ...
            map.insert("s_3_error_output_prefix".to_string(), self.r#s_3_error_output_prefix.to_pulumi_value().await);
            map.insert("table_name".to_string(), self.r#table_name.to_pulumi_value().await);
            map.insert("unique_keys".to_string(), self.r#unique_keys.to_pulumi_value().await);
