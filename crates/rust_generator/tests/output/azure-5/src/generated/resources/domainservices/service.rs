...
pub mod service {
    ...
    fn __create(
        ...
        let filtered_sync_enabled_binding = args.filtered_sync_enabled.get_output(ctx);
        let initial_replica_set_binding = args.initial_replica_set.get_output(ctx);
        let location_binding = args.location.get_output(ctx);
        let name_binding = args.name.get_output(ctx);
        let notifications_binding = args.notifications.get_output(ctx);
        let resource_group_name_binding = args.resource_group_name.get_output(ctx);
        let secure_ldap_binding = args.secure_ldap.get_output(ctx);
