...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for GetCatalogTableStorageDescriptorSchemaReferenceSchemaId {
    ...
    > {
        async move {
            ...
            map.insert("registry_name".to_string(), self.r#registry_name.to_pulumi_value().await);
            map.insert("schema_arn".to_string(), self.r#schema_arn.to_pulumi_value().await);
            map.insert("schema_name".to_string(), self.r#schema_name.to_pulumi_value().await);
