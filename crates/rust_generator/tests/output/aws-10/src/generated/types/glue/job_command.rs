...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for JobCommand {
    ...
    > {
        async move {
            ...
            map.insert("python_version".to_string(), self.r#python_version.to_pulumi_value().await);
            map.insert("runtime".to_string(), self.r#runtime.to_pulumi_value().await);
            map.insert("script_location".to_string(), self.r#script_location.to_pulumi_value().await);
