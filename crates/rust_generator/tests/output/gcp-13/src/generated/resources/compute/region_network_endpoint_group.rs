...
pub mod region_network_endpoint_group {
    ...
    fn __create(
        ...
        let app_engine_binding = args.app_engine.get_output(ctx);
        let cloud_function_binding = args.cloud_function.get_output(ctx);
        let cloud_run_binding = args.cloud_run.get_output(ctx);
        let description_binding = args.description.get_output(ctx);
        let name_binding = args.name.get_output(ctx);
        let network_binding = args.network.get_output(ctx);
        let network_endpoint_type_binding = args.network_endpoint_type.get_output(ctx);
        let project_binding = args.project.get_output(ctx);
        let psc_data_binding = args.psc_data.get_output(ctx);
        let psc_target_service_binding = args.psc_target_service.get_output(ctx);
        let region_binding = args.region.get_output(ctx);
        let serverless_deployment_binding = args.serverless_deployment.get_output(ctx);
