...
pub mod dataset_delimited_text {
    ...
    fn __create(
        ...
        let azure_blob_fs_location_binding = args.azure_blob_fs_location.get_output(ctx);
        let azure_blob_storage_location_binding = args
            .azure_blob_storage_location
            .get_output(ctx);
        let column_delimiter_binding = args.column_delimiter.get_output(ctx);
        let compression_codec_binding = args.compression_codec.get_output(ctx);
        let compression_level_binding = args.compression_level.get_output(ctx);
        ...
        let escape_character_binding = args.escape_character.get_output(ctx);
        let first_row_as_header_binding = args.first_row_as_header.get_output(ctx);
        let folder_binding = args.folder.get_output(ctx);
        let http_server_location_binding = args.http_server_location.get_output(ctx);
        let linked_service_name_binding = args.linked_service_name.get_output(ctx);
        let name_binding = args.name.get_output(ctx);
        let null_value_binding = args.null_value.get_output(ctx);
        let parameters_binding = args.parameters.get_output(ctx);
        let quote_character_binding = args.quote_character.get_output(ctx);
        let row_delimiter_binding = args.row_delimiter.get_output(ctx);
