            ...
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    r#active_directory_auth_enabled: {
                        let field_value = match fields_map.get("activeDirectoryAuthEnabled") {
                            Some(value) => value,
                            None => bail!("Missing field 'activeDirectoryAuthEnabled' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#password_auth_enabled: {
                        let field_value = match fields_map.get("passwordAuthEnabled") {
                            Some(value) => value,
                            None => bail!("Missing field 'passwordAuthEnabled' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#tenant_id: {
