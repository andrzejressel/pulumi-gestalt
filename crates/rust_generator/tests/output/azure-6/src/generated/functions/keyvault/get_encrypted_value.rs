...
pub mod get_encrypted_value {
    ...
    pub fn invoke(
        ...
        let algorithm_binding = args.algorithm.get_output(ctx);
        let encrypted_data_binding = args.encrypted_data.get_output(ctx);
        let key_vault_key_id_binding = args.key_vault_key_id.get_output(ctx);
        let plain_text_value_binding = args.plain_text_value.get_output(ctx);
