...
pub mod managed_hardware_security_module_key_rotation_policy {
    ...
    fn __create(
        ...
        let expire_after_binding = args.expire_after.get_output(ctx);
        let managed_hsm_key_id_binding = args.managed_hsm_key_id.get_output(ctx);
        let time_after_creation_binding = args.time_after_creation.get_output(ctx);
        let time_before_expiry_binding = args.time_before_expiry.get_output(ctx);
