    ...
    fn from_pulumi_value(
        ...
        match value.content {
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    r#static_: {
                        let field_value = match fields_map.get("static") {
                            Some(value) => value,
                            None => bail!("Missing field 'static' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#template_linked: {
                        let field_value = match fields_map.get("templateLinked") {
                            Some(value) => value,
                            None => bail!("Missing field 'templateLinked' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                })
