        ...
        match value.content {
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    ...
                    r#field_actions: {
                        let field_value = match fields_map.get("fieldActions") {
                            Some(value) => value,
                            None => bail!("Missing field 'fieldActions' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#resource_filter: {
                        let field_value = match fields_map.get("resourceFilter") {
                            Some(value) => value,
                            None => bail!("Missing field 'resourceFilter' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                })
