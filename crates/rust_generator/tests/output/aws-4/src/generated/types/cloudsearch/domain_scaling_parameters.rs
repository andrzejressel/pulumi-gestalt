...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for DomainScalingParameters {
    ...
    > {
        async move {
            ...
            map.insert("desired_instance_type".to_string(), self.r#desired_instance_type.to_pulumi_value().await);
            map.insert("desired_partition_count".to_string(), self.r#desired_partition_count.to_pulumi_value().await);
            map.insert("desired_replication_count".to_string(), self.r#desired_replication_count.to_pulumi_value().await);
