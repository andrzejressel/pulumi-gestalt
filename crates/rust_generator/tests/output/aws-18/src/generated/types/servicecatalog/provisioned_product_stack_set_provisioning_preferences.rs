...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for ProvisionedProductStackSetProvisioningPreferences {
    ...
    > {
        async move {
            ...
            map.insert("max_concurrency_count".to_string(), self.r#max_concurrency_count.to_pulumi_value().await);
            map.insert("max_concurrency_percentage".to_string(), self.r#max_concurrency_percentage.to_pulumi_value().await);
