...
pub mod secret_version {
    ...
    fn __create(
        ...
        let secret_binary_binding = args.secret_binary.get_output(ctx);
        let secret_id_binding = args.secret_id.get_output(ctx);
        let secret_string_binding = args.secret_string.get_output(ctx);
        let version_stages_binding = args.version_stages.get_output(ctx);
