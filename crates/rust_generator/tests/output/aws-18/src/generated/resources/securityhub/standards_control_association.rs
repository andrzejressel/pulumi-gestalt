...
pub mod standards_control_association {
    ...
    fn __create(
        ...
        let association_status_binding = args.association_status.get_output(ctx);
        let security_control_id_binding = args.security_control_id.get_output(ctx);
        let standards_arn_binding = args.standards_arn.get_output(ctx);
        let updated_reason_binding = args.updated_reason.get_output(ctx);
