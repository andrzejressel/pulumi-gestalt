...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for MaintenanceWindowTaskTaskInvocationParametersRunCommandParametersCloudwatchConfig {
    ...
    > {
        async move {
            ...
            map.insert("cloudwatch_log_group_name".to_string(), self.r#cloudwatch_log_group_name.to_pulumi_value().await);
            map.insert("cloudwatch_output_enabled".to_string(), self.r#cloudwatch_output_enabled.to_pulumi_value().await);
