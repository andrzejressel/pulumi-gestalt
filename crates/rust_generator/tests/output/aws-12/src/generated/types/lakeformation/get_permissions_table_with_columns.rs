...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for GetPermissionsTableWithColumns {
    ...
    > {
        async move {
            ...
            map.insert("excluded_column_names".to_string(), self.r#excluded_column_names.to_pulumi_value().await);
            map.insert("name".to_string(), self.r#name.to_pulumi_value().await);
            map.insert("wildcard".to_string(), self.r#wildcard.to_pulumi_value().await);
