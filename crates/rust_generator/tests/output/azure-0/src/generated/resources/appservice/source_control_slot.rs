...
pub mod source_control_slot {
    ...
    fn __create(
        ...
        let branch_binding = args.branch.get_output(ctx);
        let github_action_configuration_binding = args
            .github_action_configuration
            .get_output(ctx);
        let repo_url_binding = args.repo_url.get_output(ctx);
        let rollback_enabled_binding = args.rollback_enabled.get_output(ctx);
        let slot_id_binding = args.slot_id.get_output(ctx);
        let use_local_git_binding = args.use_local_git.get_output(ctx);
        let use_manual_integration_binding = args.use_manual_integration.get_output(ctx);
        let use_mercurial_binding = args.use_mercurial.get_output(ctx);
