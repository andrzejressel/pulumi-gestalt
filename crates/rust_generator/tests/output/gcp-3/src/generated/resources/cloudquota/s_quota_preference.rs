...
pub mod s_quota_preference {
    ...
    fn __create(
        ...
        let contact_email_binding = args.contact_email.get_output(ctx);
        let dimensions_binding = args.dimensions.get_output(ctx);
        let ignore_safety_checks_binding = args.ignore_safety_checks.get_output(ctx);
        let justification_binding = args.justification.get_output(ctx);
        let name_binding = args.name.get_output(ctx);
        let parent_binding = args.parent.get_output(ctx);
        let quota_config_binding = args.quota_config.get_output(ctx);
