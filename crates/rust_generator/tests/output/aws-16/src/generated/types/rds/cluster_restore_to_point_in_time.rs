...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for ClusterRestoreToPointInTime {
    ...
    > {
        async move {
            ...
            map.insert("restore_to_time".to_string(), self.r#restore_to_time.to_pulumi_value().await);
            map.insert("restore_type".to_string(), self.r#restore_type.to_pulumi_value().await);
            map.insert("source_cluster_identifier".to_string(), self.r#source_cluster_identifier.to_pulumi_value().await);
            map.insert("source_cluster_resource_id".to_string(), self.r#source_cluster_resource_id.to_pulumi_value().await);
