    ...
    fn from_pulumi_value(
        ...
        match value.content {
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    r#peered_network: {
                        let field_value = match fields_map.get("peeredNetwork") {
                            Some(value) => value,
                            None => bail!("Missing field 'peeredNetwork' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#peered_network_ip_range: {
                        let field_value = match fields_map.get("peeredNetworkIpRange") {
                            Some(value) => value,
                            None => bail!("Missing field 'peeredNetworkIpRange' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                })
