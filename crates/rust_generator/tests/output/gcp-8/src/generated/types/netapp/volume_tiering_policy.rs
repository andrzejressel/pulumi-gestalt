            ...
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    r#cooling_threshold_days: {
                        let field_value = match fields_map.get("coolingThresholdDays") {
                            Some(value) => value,
                            None => bail!("Missing field 'coolingThresholdDays' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#tier_action: {
                        let field_value = match fields_map.get("tierAction") {
                            Some(value) => value,
                            None => bail!("Missing field 'tierAction' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                })
