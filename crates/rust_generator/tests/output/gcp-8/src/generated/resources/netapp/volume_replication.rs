...
pub mod volume_replication {
    ...
    fn __create(
        ...
        let destination_volume_parameters_binding = args
            ...
            .get_output(ctx);
        let force_stopping_binding = args.force_stopping.get_output(ctx);
        let labels_binding = args.labels.get_output(ctx);
        let location_binding = args.location.get_output(ctx);
        let name_binding = args.name.get_output(ctx);
        let project_binding = args.project.get_output(ctx);
        let replication_enabled_binding = args.replication_enabled.get_output(ctx);
        let replication_schedule_binding = args.replication_schedule.get_output(ctx);
        let volume_name_binding = args.volume_name.get_output(ctx);
        let wait_for_mirror_binding = args.wait_for_mirror.get_output(ctx);
