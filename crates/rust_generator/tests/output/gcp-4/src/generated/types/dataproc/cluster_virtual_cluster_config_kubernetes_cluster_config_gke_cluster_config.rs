            ...
            PulumiValueContent::Object(ref _obj) => {
                ...
                Ok(Self {
                    r#gke_cluster_target: {
                        let field_value = match fields_map.get("gkeClusterTarget") {
                            Some(value) => value,
                            None => bail!("Missing field 'gkeClusterTarget' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                    r#node_pool_targets: {
                        let field_value = match fields_map.get("nodePoolTargets") {
                            Some(value) => value,
                            None => bail!("Missing field 'nodePoolTargets' while converting PulumiValue to {}", std::any::type_name::<Self>()),
                        };
                        FromPulumiValue::from_pulumi_value(field_value)?
                    },
                })
