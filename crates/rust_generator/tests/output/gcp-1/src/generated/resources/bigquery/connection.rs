...
pub mod connection {
    ...
    fn __create(
        ...
        let azure_binding = args.azure.get_output(ctx);
        let cloud_resource_binding = args.cloud_resource.get_output(ctx);
        let cloud_spanner_binding = args.cloud_spanner.get_output(ctx);
        let cloud_sql_binding = args.cloud_sql.get_output(ctx);
