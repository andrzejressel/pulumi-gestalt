...
pub mod hci_virtual_hard_disk {
    ...
    fn __create(
        ...
        let block_size_in_bytes_binding = args.block_size_in_bytes.get_output(ctx);
        let custom_location_id_binding = args.custom_location_id.get_output(ctx);
        let disk_file_format_binding = args.disk_file_format.get_output(ctx);
        let disk_size_in_gb_binding = args.disk_size_in_gb.get_output(ctx);
        let dynamic_enabled_binding = args.dynamic_enabled.get_output(ctx);
