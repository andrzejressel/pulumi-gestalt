...
pub mod origin_ca_certificate {
    ...
    fn __create(
        ...
        let hostnames_binding = args.hostnames.get_output(ctx);
        let min_days_for_renewal_binding = args.min_days_for_renewal.get_output(ctx);
        let request_type_binding = args.request_type.get_output(ctx);
        let requested_validity_binding = args.requested_validity.get_output(ctx);
