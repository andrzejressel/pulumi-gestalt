...
pub mod vpn_site {
    ...
    fn __create(
        ...
        let address_cidrs_binding = args.address_cidrs.get_output(ctx);
        let device_model_binding = args.device_model.get_output(ctx);
        let device_vendor_binding = args.device_vendor.get_output(ctx);
        let links_binding = args.links.get_output(ctx);
        let location_binding = args.location.get_output(ctx);
        let name_binding = args.name.get_output(ctx);
        let o365_policy_binding = args.o365_policy.get_output(ctx);
        let resource_group_name_binding = args.resource_group_name.get_output(ctx);
        let tags_binding = args.tags.get_output(ctx);
        let virtual_wan_id_binding = args.virtual_wan_id.get_output(ctx);
