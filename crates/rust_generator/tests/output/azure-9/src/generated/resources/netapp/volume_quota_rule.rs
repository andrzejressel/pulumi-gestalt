...
pub mod volume_quota_rule {
    ...
    fn __create(
        ...
        let quota_size_in_kib_binding = args.quota_size_in_kib.get_output(ctx);
        let quota_target_binding = args.quota_target.get_output(ctx);
        let quota_type_binding = args.quota_type.get_output(ctx);
        let volume_id_binding = args.volume_id.get_output(ctx);
