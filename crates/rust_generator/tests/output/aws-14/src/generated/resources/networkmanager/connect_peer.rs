...
pub mod connect_peer {
    ...
    fn __create(
        ...
        let bgp_options_binding = args.bgp_options.get_output(ctx);
        let connect_attachment_id_binding = args.connect_attachment_id.get_output(ctx);
        let core_network_address_binding = args.core_network_address.get_output(ctx);
        let inside_cidr_blocks_binding = args.inside_cidr_blocks.get_output(ctx);
        let peer_address_binding = args.peer_address.get_output(ctx);
        let subnet_arn_binding = args.subnet_arn.get_output(ctx);
