...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for BudgetActionDefinitionIamActionDefinition {
    ...
    > {
        async move {
            ...
            map.insert("policy_arn".to_string(), self.r#policy_arn.to_pulumi_value().await);
            map.insert("roles".to_string(), self.r#roles.to_pulumi_value().await);
            map.insert("users".to_string(), self.r#users.to_pulumi_value().await);
