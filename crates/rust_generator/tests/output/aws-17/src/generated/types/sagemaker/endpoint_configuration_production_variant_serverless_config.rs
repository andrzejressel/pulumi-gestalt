...
impl pulumi_gestalt_rust::__private::pulumi_gestalt_model::ToPulumiValue for EndpointConfigurationProductionVariantServerlessConfig {
    ...
    > {
        async move {
            ...
            map.insert("max_concurrency".to_string(), self.r#max_concurrency.to_pulumi_value().await);
            map.insert("memory_size_in_mb".to_string(), self.r#memory_size_in_mb.to_pulumi_value().await);
            map.insert("provisioned_concurrency".to_string(), self.r#provisioned_concurrency.to_pulumi_value().await);
