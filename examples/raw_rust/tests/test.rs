...
fn test_integration() -> Result<()> {
    ...
    assert!(
        resource_provider_id.starts_with("urn:pulumi:test::Pulumi-raw-rust-Sample::pulumi:providers:random::custom-provider::")
    );
    ...
    assert_eq!(
        resource_urn,
        "urn:pulumi:test::Pulumi-raw-rust-Sample::random:index/randomString:RandomString::my_name"
    );
